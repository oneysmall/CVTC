...
def resnet_v1_views(inputs,blocks,dataset_name,num_classes=None,is_training=True,global_pool=True,output_stride=None,include_root_block=True,spatial_squeeze=True,reuse=None,scope=None):
    ...
    with tf.variable_scope(scope, 'resnet_v1', [inputs], reuse=reuse) as sc:
        ...
        with slim.arg_scope([slim.conv2d, bottleneck, resnet_utils_views.stack_blocks_dense], outputs_collections=end_points_collection):
            with slim.arg_scope([slim.batch_norm, slim.dropout], is_training=is_training):
                ...
                net, end_points = resnet_utils_views.stack_blocks_dense(net, blocks, dataset_name, output_stride,)
                if global_pool:
...
def resnet_v1_50_views(inputs,num_classes=None,is_training=True,global_pool=True,output_stride=None,reuse=None,scope='resnet_v1_50',dataset_name='isnotVehicleID'):
    """ResNet-50 model of [1]. See resnet_v1() for arg and return description."""
    blocks = [resnet_utils_views.Block('block1', bottleneck, [(256, 64, 1)] * 2 + [(256, 64, 2)]),resnet_utils_views.Block('block2', bottleneck, [(512, 128, 1)] * 3 + [(512, 128, 2)]),resnet_utils_views.Block('block3', bottleneck, [(1024, 256, 1)] * 5 + [(1024, 256, 2)]),resnet_utils_views.Block('block4', bottleneck, [(2048, 512, 1)] * 3)
	]
    return resnet_v1_views(inputs, blocks, dataset_name, num_classes, is_training, global_pool=global_pool, output_stride=output_stride,include_root_block=True, reuse=reuse, scope=scope)
