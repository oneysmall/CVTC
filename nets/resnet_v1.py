...
def residual_unit(x, ci, co, name, stride=1):
    with tf.name_scope(name), tf.variable_scope(name):
        # 1x1 Convolution
        conv_1=slim.conv2d(x,co/4,[1,1],stride=1,padding='SAME',activation_fn=None,normalizer_fn=slim.batch_norm,biases_initializer=None,scope='conv_1')
        # 3x3 Convolution
        conv_2=slim.conv2d(conv_1,co/4,[3,3],stride=stride,padding='SAME',activation_fn=None,normalizer_fn=slim.batch_norm,biases_initializer=None,scope='conv_2')
        # 1x1 Convolution
        conv_3=slim.conv2d(conv_2,co,[1,1],stride=1,padding='SAME',activation_fn=None,normalizer_fn=slim.batch_norm,biases_initializer=None,scope='conv_3')
        # Skip connection
        if co != ci or stride > 1:
            skip=slim.conv2d(conv_3,co,[1,1],stride=stride,activation_fn=None,normalizer_fn=slim.batch_norm,biases_initializer=None,scope='conv_skip')
        else:
            skip = x
...
def upsample(x, name, size):
    with tf.name_scope(name):
        ...
        return outputs      
def attention_module(x, ci, name, p=1, t=2, r=1):
    with tf.name_scope(name), tf.variable_scope(name):
        ...
        with tf.name_scope("mask_branch"), tf.variable_scope("mask_branch"):
            ...
            # Max pooling
            masks_1=slim.max_pool2d(pre_pros,[3,3],stride=2,scope='pool_1')
            for idx in range(r):
                unit_name = "mask_res1_{}".format(idx + 1)
                masks_1 = residual_unit(masks_1, ci, ci, unit_name)
            size_2 = masks_1.get_shape().as_list()[1:3]
            # Max pooling
            masks_2=slim.max_pool2d(masks_1,[3,3],stride=2,scope='pool_2')
            for idx in range(2 * r):
                unit_name = "mask_res2_{}".format(idx + 1)
                masks_2 = residual_unit(masks_2, ci, ci, unit_name)
            # Interpolation
            masks_3 = upsample(masks_2, "inter_1", size_2)
            # Skip connection
            skip = residual_unit(masks_1, ci, ci, "skip")
            masks_3 = tf.add(masks_3, skip, name="fuse_add")
            for idx in range(r):
                unit_name = "mask_res3_{}".format(idx + 1)
                masks_3 = residual_unit(masks_3, ci, ci, unit_name)
            # Interpolation
            masks_4 = upsample(masks_3, "inter_2", size_1)
            # Batch Normalization
            #masks_4 = self.batch_normal(masks_4, self.is_train, "mask_bn1", tf.nn.relu)
            # 1x1 Convolution
            masks_4=slim.conv2d(masks_4,ci,[1,1],stride=1,activation_fn=None,normalizer_fn=slim.batch_norm,biases_initializer=None,scope='mask_conv1')
            # 1x1 Convolution
            masks_4=slim.conv2d(masks_4,ci,[1,1],stride=1,activation_fn=None,normalizer_fn=slim.batch_norm,biases_initializer=None,scope='mask_conv2')
            # Sigmoid
        ...
        with tf.name_scope("post_processing"), tf.variable_scope("post_processing"):
            for idx in range(p):
                ...
                outputs = residual_unit(outputs, ci, ci, unit_name)
    return outputs   
...
def resnet_v1(inputs,blocks,num_classes=None,is_training=True,global_pool=True,output_stride=None,include_root_block=True,spatial_squeeze=True,reuse=None,scope=None):
    ...
    with tf.variable_scope(scope, 'resnet_v1', [inputs], reuse=reuse) as sc:
        end_points_collection = sc.name + '_end_points'  #end_points_collection=resnet_v1_50_end_points
...
def resnet_v1_50(inputs,num_classes=None,is_training=True,global_pool=True,output_stride=None,reuse=None, scope='resnet_v1_50', dataset_name='isnotVehicleID'):
