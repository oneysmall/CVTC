...
class VeidDataset(Dataset):
    FILE_BY_PART = {'train': 'train', 'test': 'test', 'query': 'query'}  
    
    def __init__(self, data_directory, dataset_part, mean=None, std=None, num_classes=None, augment=True, png=True):
        if mean is None:
            mean = [107.460081, 107.206399, 108.914598] #all
        if std is None:
            std = [54.710137, 54.548842, 54.995074] #all
    ...
    def get_input_data(self, is_training):
        ...
        colors = [self.get_color_from_path(image_path) for image_path in image_paths]
        types = [self.get_type_from_path(image_path) for image_path in image_paths]
        
        print('Read %d image paths for processing for dataset_part: %s' % (len(image_paths), self._dataset_part))
        return image_paths, file_names, actual_labels, labels, cameras,colors,types
    ...
    def prepare_sliced_data_for_batching(self, sliced_input_data, image_size):
        image_path_tensor, file_name_tensor, actual_label_tensor, label_tensor, camera_tensor, color_tensor, type_tensor = sliced_input_data
        image_tensor = self.read_and_distort_image(file_name_tensor, image_path_tensor, image_size)
    
        return self.get_dict_for_batching(actual_label_tensor=actual_label_tensor, camera_tensor=camera_tensor, file_name_tensor=file_name_tensor, image_path_tensor=image_path_tensor,image_tensor=image_tensor, label_tensor=label_tensor, color_tensor=color_tensor, type_tensor=type_tensor)
    ...
    def get_input_function_dictionaries(self, batched_input_data):
        ...
        {'labels': batched_input_data['label'], 'actual_labels': batched_input_data['actual_label'], 'cameras': batched_input_data['camera'], 'colors':batched_input_data['color'], 'types':batched_input_data['type']}
    ...
    def get_camera_from_path(path):
        ...
        return int(camera_sequence_string[1:4])
    ...
    def get_color_from_path(path):
        filename = ntpath.basename(path)
        color_label = filename.split('_')[4] 
        return int(color_label[1:3])

    @staticmethod
    def get_type_from_path(path):
        filename = ntpath.basename(path)
        type_label = filename.split('_')[5] 
        return int(type_label[1])
    #-----------------------------------------------------------------------------------------------------------------------------------
